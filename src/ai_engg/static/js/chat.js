...
function updateTokenUsage(usedTokens, maxTokens) {
    ...
    // Update the text
    tokenCounter.textContent = `Total used: ${formattedUsed} / ${formattedMax} (${(usedTokens/maxTokens*100).toFixed(2)}%)`;
    
    // Update the progress bar
    const percentage = Math.min(100, (usedTokens / maxTokens) * 100);
    const progressBarContainer = tokenCounter.parentElement;
    if (progressBarContainer) {
        const progressBar = progressBarContainer.querySelector('.h-2 > div');
        if (progressBar) {
...
document.getElementById('chat-form').addEventListener('submit', async (e) => {
    ...
    if (currentImageData) {
        ...
        // It's safer to use currentMediaType if available, otherwise default to 'image/jpeg'
    ...
    try {
        ...
        if (data.token_usage) {
            updateTokenUsage(
                ...
                data.token_usage.max_tokens || 200000
            );
        } else {
            console.warn('No token_usage in response:', data);
        }
        ...
        if (data && data.response) {
            ...
            // This else block handles cases where the server responded, but with no 'response' field
            appendMessage('Error: No response received from server');
        }
